...
inicio:- %Inicio de nuestro programa, creando el objeto de dialogo con la variable D
   ...
   new(Label5,label(text,'        Sistema de orientación sobre las carreras en la UNNOBA')),
	send(Label5,colour,blue),
   new(Label6,label(text,'                                    ¡ Muchos Éxitos !')),
...
ingenieria_en_informatica:-carrera_ingenieria_en_informatica,
	%consultar('¿Te interesa analizar, diseñar e implementar sistemas de información para optimizar procesos y facilitar la toma de decisiones?'),
	consultar('¿Te atrae la idea de colaborar en la definición de políticas y estrategias informáticas en una organización?'),
	consultar('¿Te gusta la idea de diseñar y administrar redes de computadoras, asegurando una comunicación eficiente y segura?'),
	consultar('¿Te atrae la investigación y desarrollo en el campo de la informática, buscando soluciones innovadoras para problemas tecnológicos?').

licenciatura_en_genetica:-carrera_genetica,
	%consultar('¿Te sentis intrigado por la posibilidad de contribuir al desarrollo de productos a través de la comprensión y aplicación de los principios genéticos,que rigen la herencia y la variabilidad en los organismos vivos?'),
	consultar('¿Sentís curiosidad por realizar estudios e investigaciones en áreas como biología ,genética molecular, vegetal, animal y microbiología, explorando los secretos del ADN cómo determina las características de los seres vivos?'),
	consultar('¿Te atrae la idea de participar en investigaciones para mejorar la producción agropecuaria,aplicando los principios de la genética para optimizar la salud y productividad de las plantas y animales?'),
        consultar('¿Te interesa desarrollar sistemas de diagnóstico de laboratorio relacionados con la sanidad humana, animal y vegetal, basados en el análisis de material genético para comprender y prevenir como por ejemplo enfermedades hereditarias?').
...
ingenieria_en_alimentos:-carrera_alimentos,
	...
	%en establecimientos industriales y comerciales involucrados en la producción y procesamiento de alimentos?'),
	consultar('¿Sientes interés en diseñar, implementar y controlar sistemas de procesamiento industrial de alimentos,
        así como en investigar y desarrollar nuevas técnicas en la fabricación y transformación de productos alimenticios?'),
	consultar('¿Te gusta la idea de diseñar, implementar, controlar y supervisar operaciones relacionadas con el control
        de calidad de materias primas, productos en elaboración y productos finales en la industria alimentaria?'),
        consultar('¿Te interesa participar en la realización de estudios relacionados con el saneamiento ambiental,
...
abogacia:-carrera_abogacia,
	...
	consultar('¿Te resulta intrigante la idea de brindar opiniones jurídicas en diversos temas legales?').

contador:-carrera_contador,
        %consultar('¿Te gusta gestionar políticas, estrategias y proyectos, así como participar en el diseño,
	%ejecución y evaluación de funciones de planificación y organización en diversas organizaciones?'),
	consultar('¿Te atrae la idea de participar en la definición de objetivos y metas de las organizaciones,
        desarrollar cursos de acción y evaluar el impacto económico, social y ambiental de tus decisiones?'),
	consultar('¿Te interesa asesorar, planificar y coordinar actividades económicas, patrimoniales, financieras e impositivas en organizaciones?'),
	consultar('¿Te gusta trabajar con números y estar involucrado/a en el asesoramiento financiero y económico de empresas?').
...
carrera_ingenieria_en_informatica:-consultar('¿Te interesa analizar, diseñar e implementar sistemas de información para optimizar procesos y facilitar la toma de decisiones?'),!.
carrera_genetica:-consultar('¿Te sentis intrigado por la posibilidad de contribuir al desarrollo de productos a través de la comprensión y aplicación de los principios genéticos
...
carrera_contador:-consultar('¿Te gusta gestionar políticas, estrategias y proyectos, así como participar en el diseño,
ejecución y evaluación de funciones de planificación y organización en diversas organizaciones?'),!.
...
mostrar_preguntas_respuestas(Men) :-
    ...
    % Mostrar la pregunta y la respuesta en el diálogo
    send(Men, append, label(text, Pregunta)),
    send(Men, append, label(text, Respuesta)),
    fail. % Continuar buscando más soluciones
mostrar_preguntas_respuestas(_). % Detenerse cuando no haya más soluciones
...
main1:-
    ...
    new(@texto,label(text,'Guía básica para saber el funcionamiento de la página de Prolog:')),
    ...
    new(@texto5,label(text,'Contesta las preguntas de acuerdo con las habilidades y aptitudes que consideres\n que tienes y pones en práctica.')),
    new(@texto6,label(text,'\nDespués se te mostrarán tus resultados de acuerdo con la carrera que consideremos más apta para ti.')),
...
main2:-
    new(D4, dialog('Información sobre las Carreras', size(600, 500))),
    send(D4, append, label(text, 'Información sobre las Carreras',bold)),
    send(D4, append, label(text, 'Ingeniería en Informática:',bold)),
    send(D4, append, label(text, ' *Planificar, dirigir, realizar y/o evaluar sistemas de apoyo al diagnóstico por imágenes; sistemas de procesamiento de imágenes satelitales y sonido.')),
    send(D4, append, label(text, ' *Diseñar, implementar y controlar la calidad de software.')),
    send(D4, append, label(text, ' *Desempeñarse en organizaciones o de manera independiente, en el mantenimiento de procesos automáticos de datos y software específico de comunicaciones de datos.')),
    send(D4, append, label(text, ' *Dirigir áreas de informática.')),
    send(D4, append, label(text, '')), % Agrega un espacio en blanco para separar las descripciones

    send(D4, append, label(text, 'Licenciatura en Genética:',bold)),
    send(D4, append, label(text, ' *Desarrollar productos generados por manipulación genética.')),
    send(D4, append, label(text, ' *Realizar estudios e investigaciones referidos a la biología, genética molecular, vegetal, animal y microbiológica.')),
    send(D4, append, label(text, ' *Participar en la realización de estudios e investigaciones a los efectos del mejoramiento de la producción agropecuaria.')),
    send(D4, append, label(text, ' *Desarrollar sistemas de diagnóstico de laboratorio en el ámbito de la sanidad humana, animal y vegetal, basados en los análisis de material genético.')),    send(D4, append, label(text, '')), % Agrega un espacio en blanco para separar las descripciones
    % Información para Abogado
    send(D4, append, label(text, 'Abogado:',bold)),
    send(D4, append, label(text, '* Ejercer la representación legal – judicial y extrajudicial y el patrocinio jurídico de personas y entidades públicas y/o privadas ante el Poder Judicial Nacional o Provincial, en todas las ramas del Derecho público y privado.')),
    ...
    send(D4, append, label(text, ' *Asesorar en todo asunto que requiera opinión jurídica.')),
    send(D4, append, label(text, '')), % Agrega un espacio en blanco para separar las descripciones

    % Información para Contador Público
    send(D4, append, label(text, 'Contador Público:',bold)),
    send(D4, append, label(text, ' *Asesorar, planificar y coordinar la ejecución y el control de actividades inherentes a aspectos económicos, patrimoniales, financieros e impositivos en cualquier tipo de organización pública o privada.')),
    send(D4, append, label(text, ' *Actuar como asesor/a o consultor/a en la práctica independiente.')),
    send(D4, append, label(text, ' *Otorgar fe pública del estado económico, financiero y patrimonial de personas físicas y jurídicas.')),
    send(D4, append, label(text, ' *Emitir opinión sobre la razonabilidad de los estados contables e informes de sindicatura y auditoría.')),
    send(D4, append, label(text, ' *Actuar en el ámbito de la justicia laboral y societaria.')),
    send(D4, append, label(text, '')),

    send(D4, append, label(text, 'Ingeniería en Alimentos:', bold)),
    send(D4, append, label(text, ' *Proyectar, planificar, dirigir y controlar las instalaciones, maquinarias e instrumentos de establecimientos industriales y/o comerciales en los que se involucre la fabricación, conservación, transformación, fraccionamiento, envasado, transporte y almacenaje de productos alimenticios.')),
    send(D4, append, label(text, ' *Diseñar, implementar y controlar sistemas de procesamiento industrial de alimentos; Investigar y desarrollar técnicas de fabricación, transformación y fraccionamiento.')),
    send(D4, append, label(text, ' *Diseñar, implementar, controlar y supervisar las operaciones correspondientes al control de calidad de las materias primas a procesar, los productos en elaboración y los productos elaborados en la industria alimentaria.')),
    send(D4, append, label(text, ' *Participar en la realización de estudios relativos al saneamiento ambiental, seguridad e higiene en la industria alimentaria.')),
